...
# %%
l0, l1, l2, l3, l4, l5, l6, l7, l8, l9, l10, l11 = list(),  list(), list(
...
ouput_sorted = ouput_df.sort_values(
    ['avg market cap', 'currency'], ascending=False)
...
# %%
print(currencies.loc[currencies['currency code'] ==
                     currencies_list[ouput_df['profit'].idxmin()]])
ouput_df_wo_ol = ouput_df.drop(ouput_df['profit'].idxmin(), axis=0)

print(currencies.loc[currencies['currency code'] ==
                     currencies_list[ouput_df_wo_ol['profit'].idxmin()]])
...
plt.ylabel('profit')
plt.savefig(PATH_TO_SAVE.format('outlier_profit'))
plt.show()
...
plt.savefig(PATH_TO_SAVE.format('outlier_profit_per_day'))
plt.show()


# %%
ouput_df_wo_olx = ouput_df_wo_ol.drop(
    ouput_df_wo_ol['avg market cap'].idxmax(), axis=0)
ouput_df_wo_olx = ouput_df_wo_olx.drop(
    ouput_df_wo_olx['avg market cap'].idxmax(), axis=0)
ouput_df_wo_olx = ouput_df_wo_olx.drop(
    ouput_df_wo_olx['avg market cap'].idxmax(), axis=0)
...
plt.ylabel('std')
plt.savefig(PATH_TO_SAVE.format('std_market_cap'))
plt.show()
...
plt.xlabel('std')
plt.ylabel('profit')
plt.savefig(PATH_TO_SAVE.format('std_profit'))
plt.show()
...
plt.xlabel('std')
plt.ylabel('avg profit per day')
plt.savefig(PATH_TO_SAVE.format('std_profit_per_day'))
plt.show()
...
plt.savefig(PATH_TO_SAVE.format('hist_outlier'))
plt.show()
...
plt.ylabel('observations')
plt.savefig(PATH_TO_SAVE.format('hist_corr_BTC'))
plt.show()
...
# %%
used_currencies = currencies.loc[currencies['currency code'].isin(
    currencies_list)]
